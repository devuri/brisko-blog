	...
	function brisko_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<div class="cat-links entry-meta %2$s">' . esc_html__( 'Posted in %1$s', 'brisko' ) . '</div>', $categories_list, briskoblog_display_post_categories() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			...
			if ( $tags_list ) {
				...
				printf( '<br/><span class="tags-links %2$s">' . esc_html__( 'Tags: %1$s ', 'brisko' ) . '</span>', $tags_list, briskoblog_display_tags() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
